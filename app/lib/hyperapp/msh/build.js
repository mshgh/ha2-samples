export const build = (operations, { focus, namespace, buildDi, asValue } = {}) => {
  ...
  const toValue = (props, value = props?.[asValue]) => typeof value === 'undefined' ? props : value
  ...
  const focusAction = (actionFocus, action, { curriedDi, di, props: buildProps } = {}) => {
    ...
    return (state, actionProps) => {
      ...
      const result = action(subState, inject && !curriedDi && props !== actionProps ? { ...inject, ...props } : toValue(props))
