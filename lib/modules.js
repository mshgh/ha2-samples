...
export function module(slice, data) {
  const { init, actions, view, add } = _module(slice, data)
  return { init, actions, view, add }
}

export function ns(slice) {
  const { init, add } = _module(slice)
  return { init, add }
}
...
export function ar(slice, getModule) {
  const actions = {
    ...
    add(slice, props) {
      ...
      const id = `m${seq}`
      const instance = add(getModule(props), id, props)
  ...
  const modules = [], { name, init, add, ...module } = _module(slice, {
    init: { seq: -1, order: [] },
    actions
  })
  ...
  return {
    ...
    add(props) {
      updateInit(actions.add, props)
    },
    del(idx) {
      updateInit(actions.del, idx)
    }
  }
