...
function mapModule(name, init, { init: dataInit = {}, actions: dataActions, ...data }, sliceMap) {
  const actions = mapActions(sliceMap.mapAction, dataActions)
  const childInit = init[name] = dataInit
  ...
  return {
    ...
    subModule(Module, name, props) {
      return Module(function module({ intercept, ...data } = {}) {
        return mapModule(name, childInit, data, mapSlice(name, intercept, sliceMap))
      }, props)
    }
  }
}

function _module(slice, { intercept, ...data } = {}) {
  return typeof slice !== 'string' ? slice(data)
    : mapModule(slice, {}, data, mapSlice(slice, intercept))
}
