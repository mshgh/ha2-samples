...
function mapModule(init, data, sliceMap, topLevelInit = init) {
  ...
  return {
    ...
    actions,
    views(state, props) {
      return data.views({ slice: sliceMap.extract(state), actions, state }, props)
    },
    subModule(Module, slice, props) {
      return Module(function module(data = {}) {
        return mapModule(childInit, data, mapSlice(slice, data.intercept, sliceMap), topLevelInit)
      }, props)
    }
  }
}
...
export function module(slice, data = {}) {
  const { init, actions, views, subModule } = _module(slice, data)
  return { init, actions, views, subModule }
}
